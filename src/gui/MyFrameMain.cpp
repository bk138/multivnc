...
void MyFrameMain::machine_connect(wxCommandEvent &event)
{
    ...
    if(dialog_new_connection.ShowModal() == wxID_OK && dialog_new_connection.getHost() != wxEmptyString) {
        ...
        conn_spawn("vnc://" + dialog_new_connection.getHost()
                   ...
                   + "&SshPort=" + wxString::Format(wxT("%i"), dialog_new_connection.getSshPort())
...
void MyFrameMain::bookmarks_add(wxCommandEvent &event)
{
  ...
  if(name != wxEmptyString)
      ...
      if(cfg->Exists(G_BOOKMARKS + name))
	{
	  ...
	  if(!store.Save("MultiVNC/Bookmarks/" + wxString(c->getUserName().IsEmpty() ? "" : c->getUserName() + "@") + c->getServerHost() + ":" + c->getServerPort(), c->getUserName(), c->getPassword()))
